...
class Validator {
    ...
    validate(schemaId, params) {
        ...
        if (!validator) {
            throw createError("ProtocolError", `Schema '${schemaId}' is missing from subprotocol schema '${this._subprotocol}'`);
        }
        else{
            ...
            if (!res && validator.errors?.length > 0) {
                ...
                throw createError(rpcErrorCode, this._ajv.errorsText(validator.errors), {
